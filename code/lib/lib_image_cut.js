...
const cutImage = async function(dir_name, file_name, save_cut_zeiger = false, save_cut_ziffer = false, name = "")
{
    var source = cv_lib.imread(dir_name + '/' + file_name);
    cv_lib.imwrite(dir_name + '/org.jpg', source);

    target = rotateImage(source, alignment_rotation_angle);
    cv_lib.imwrite(dir_name + '/rot.jpg', source);
    
    target = Alignment(target);
    cv_lib.imwrite(dir_name + '/alg.jpg', target);

    var zeiger = [];
    zeiger = cutZeiger(target, dir_name);

    var ziffern = [];
    ziffern = cutZiffern(target, dir_name);
...
function getRefKoordinaten_lib(src, ref)
{
  ...
  rt = new cv_lib.Point(x, y);
  return rt;
}

function cutZeiger(source, dir_name)
{
    var target;
    var name_zeiger = []
    for (i = 0; i < Analog_Counter.length; ++i)
    {
        ...
        name_zeiger[i] = dir_name + '/' + Analog_Counter[i] + '.jpg'
...
function cutZiffern(source, dir_name)
{
    var target;
    var name = []
    for (i = 0; i < Digital_Digit.length; ++i)
    {
        ...
        name[i] = dir_name + '/' + Digital_Digit[i] + '.jpg'
